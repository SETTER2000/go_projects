...
func main() {
	...
	for row := range bigDigits[0] {
		var line string
		for column := range stringOfDigits { // индекс символа в строке
			digit := stringOfDigits[column] - '0' // цифра заданная в строке, согласно индекса
			if digit <= 9 {
		...
		if row == 0 && on {
			bar = strings.Repeat("*", len(line)-2)
			fmt.Println(bar)
			defer fmt.Println(bar)
		}
		fmt.Println(line)
	}
	//if on { fmt.Println(bar) }
}
